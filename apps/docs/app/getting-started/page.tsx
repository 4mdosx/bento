...
const install = 'npx bento-kit@0.1.0-preview.1 init my-app\ncd my-app\nnpm run dev'
...
export default function GettingStarted() {
  ...
  return <DocsLayout navigation={navigation}>
    ...
    <section id="install"><h3>1. 初始化项目</h3><CodeBlock>{install}</CodeBlock><p>`init` 会创建 git 仓库、Next.js App Router 模板、Bento 依赖，并写入 button、form、dashboard-shell、list 与可运行的 List 启动页。</p></section>
    <section id="model"><h3>2. 替换业务 Model 与 Action</h3><CodeBlock>{model}</CodeBlock><p>把 `app/page.tsx` 中的示例换成你的资源类型。Action 必须传递并响应 AbortSignal。不要在 View 中直接请求数据。</p></section>
